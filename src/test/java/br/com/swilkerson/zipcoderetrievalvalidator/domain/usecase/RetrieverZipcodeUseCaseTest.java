...
class RetrieverZipcodeUseCaseTest {
    ...
    void shouldThrowInvalidZipcodeWithShortZipcode() {
        when(zipcodeValidator.execute(INVALID_ZIPCODE_SHORT)).thenThrow(InvalidZipcodeException.class);
        ...
        verify(zipcodeValidator, times(ONCE)).execute(INVALID_ZIPCODE_SHORT);
    }

    @Test
    void shouldThrowInvalidZipcodeWithLargeZipcode() {
        when(zipcodeValidator.execute(INVALID_ZIPCODE_LARGE)).thenThrow(InvalidZipcodeException.class);

        assertThatThrownBy(() -> useCase.execute(INVALID_ZIPCODE_LARGE)).isInstanceOf(InvalidZipcodeException.class);

        verifyNoInteractions(dataProvider);
        verify(zipcodeValidator, times(ONCE)).execute(INVALID_ZIPCODE_LARGE);
    }

    @Test
    void shouldThrowInvalidZipcodeWithInvalidCharOnZipcode() {
        when(zipcodeValidator.execute(INVALID_ZIPCODE_INVALID_CHAR)).thenThrow(InvalidZipcodeException.class);

        assertThatThrownBy(() -> useCase.execute(INVALID_ZIPCODE_INVALID_CHAR))
                .isInstanceOf(InvalidZipcodeException.class);

        verifyNoInteractions(dataProvider);
        verify(zipcodeValidator, times(ONCE)).execute(INVALID_ZIPCODE_INVALID_CHAR);
    }

    @Test
    void shouldThrowInvalidZipcodeWithNullZipcode() {
        when(zipcodeValidator.execute(NULL_ZIPCODE)).thenThrow(InvalidZipcodeException.class);

        assertThatThrownBy(() -> useCase.execute(NULL_ZIPCODE))
                .isInstanceOf(InvalidZipcodeException.class);

        verifyNoInteractions(dataProvider);
        verify(zipcodeValidator, times(ONCE)).execute(NULL_ZIPCODE);
    }

    @Test
    void shouldThrowNotFoundWithInvalidZipcodeValidPattern() {
        when(zipcodeValidator.execute(NOT_FOUND_ZIPCODE_WITH_DASH)).thenReturn(VALID_ZIPCODE);
        ...
        verify(zipcodeValidator, times(ONCE)).execute(NOT_FOUND_ZIPCODE_WITH_DASH);
    ...
    void shouldThrowNotFoundWithInvalidZipcodeNoDashValidPattern() {
        when(zipcodeValidator.execute(NOT_FOUND_ZIPCODE_WITHOUT_DASH)).thenReturn(VALID_ZIPCODE);
        ...
        verify(zipcodeValidator, times(ONCE)).execute(NOT_FOUND_ZIPCODE_WITHOUT_DASH);
        verify(dataProvider, times(ONCE)).execute(NOT_FOUND_ZIPCODE_WITHOUT_DASH);
    }

    @Test
    void shouldFoundZipcodeWithValidPattern() {
        when(zipcodeValidator.execute(VALID_ZIPCODE_WITH_DASH)).thenReturn(VALID_ZIPCODE);
        ...
        verify(zipcodeValidator, times(ONCE)).execute(VALID_ZIPCODE_WITH_DASH);
        verify(dataProvider, times(ONCE)).execute(VALID_ZIPCODE_WITH_DASH);
    }

    @Test
    void shouldFoundZipcodeWithValidPatternNoDash() {
        when(zipcodeValidator.execute(VALID_ZIPCODE_WITHOUT_DASH)).thenReturn(VALID_ZIPCODE);
        ...
        verify(zipcodeValidator, times(ONCE)).execute(VALID_ZIPCODE_WITHOUT_DASH);
